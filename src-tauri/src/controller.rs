...
impl AppController {
    ...
    pub fn show<R: Runtime>(&self, app: &AppHandle<R>) -> tauri::Result<()> {
        ...
        let id = monitor
            .name()
            .cloned()
            .unwrap_or_else(|| format!("primary@{},{}", position.x, position.y));
        let descriptor = DisplayDescriptor {
            id: DisplayId::new(id).map_err(|error| tauri::Error::Anyhow(error.into()))?,
            origin: DisplayPoint {
                x: position.x,
                y: position.y,
            },
        ...
        let snapshot =
            DisplaySnapshot::one(descriptor).map_err(|error| tauri::Error::Anyhow(error.into()))?;
    ...
    fn transition<R: Runtime>(
        ...
        app.emit("overlay-mode-changed", mode)?;
        if let Some(viewport) = state.snapshot.viewport.clone() {
            app.emit("overlay-viewport-changed", viewport)?;
        }
        Ok(())
    }
